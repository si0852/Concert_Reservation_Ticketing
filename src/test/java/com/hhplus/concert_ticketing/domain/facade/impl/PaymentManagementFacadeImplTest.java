...
class PaymentManagementFacadeImplTest {
    ...
    void payment_success()  throws Exception{
        ...
        Customer customer = new Customer("si", 19000.0);
        when(customerService.getCustomerData(anyLong())).thenReturn(customer);


        Payment expectedPayment = new Payment(reservationId, concertOption.getPrice(), LocalDateTime.now());
        when(paymentService.savePayment(any(Payment.class))).thenReturn(expectedPayment);

        doNothing().when(eventPublisher).publishEvent(any(PaymentEvent.class));


        Payment payment = paymentManagementFacade.paymentProgress(reservationId, token);
        assertNotNull(payment, "Payment result should not be null");

        verify(customerService).updateCharge(any(Customer.class));
        verify(concertService).updateSeatData(any(Seat.class));
        verify(reservationService).UpdateReservationData(any(Reservation.class));
